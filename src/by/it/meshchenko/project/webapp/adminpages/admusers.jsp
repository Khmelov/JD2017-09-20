...
<c:forEach items="${admUsersView.users}" var="user">
    <form class="edit-user-${user.id}" role="form" action="do?command=admusers" method="post">
      <div class="row">
        <div class="form-group col-xs-1">
          <input type="text" id="id" name="id" value="${user.id}" class="form-control input-sm" title="ID" disabled/>
          <input type="hidden" id="id" name="id" value="${user.id}" class="form-control input-sm" title="ID" />
        </div>
        <div class="form-group col-xs-3">
            <input type="text" id="name" name="name" value="${user.name}" class="form-control input-sm" disabled/>
        </div>
        <div class="form-group col-xs-3">
            <input type="text" id="password" name="password" value="${user.password}" class="form-control input-sm" disabled/>
        </div>
        <div class="form-group col-xs-3">
            <input type="text" id="phoneNumber" name="phoneNumber" value="${user.phoneNumber}" class="form-control input-sm" disabled/>
        </div>
      </div>
      <div class="margin-bottom-8"></div>
      <div class="row">
        <div class="form-group col-xs-offset-1 col-xs-4">
            <input type="text" id="email" name="email" value="${user.email}" class="form-control input-sm" disabled/>
        </div>
        <div class="form-group col-xs-3">
