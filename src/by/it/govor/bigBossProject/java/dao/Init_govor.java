...
public class Init_govor {
    ...
    public static void main(String[ ] args) {
        ...
        try (Connection connection=
            ...
            statement.executeUpdate("DROP SCHEMA IF EXISTS `govor` ;");
            statement.executeUpdate("CREATE SCHEMA IF NOT EXISTS `govor` DEFAULT CHARACTER SET utf8 ;");
            ...
            statement.executeUpdate(
                    "CREATE TABLE IF NOT EXISTS `govor`.`role` (\n" +
            ...
            statement.executeUpdate(
                    "CREATE TABLE IF NOT EXISTS `govor`.`user` (\n" +
                            ...
                            "    FOREIGN KEY (`FK_role`)\n" +
                            "    REFERENCES `govor`.`role` (`ID`)\n" +
                            "    ON DELETE RESTRICT \n" +
                            "    ON UPDATE RESTRICT )\n" +
                            "ENGINE = InnoDB;"
            );
            statement.executeUpdate(
                    "CREATE TABLE IF NOT EXISTS `govor`.`ads` (\n" +
                            "  `ID` INT NOT NULL AUTO_INCREMENT,\n" +
                            "  `RoomCount` INT NULL,\n" +
                            "  `Floor` INT NULL,\n" +
                            "  `Floors` INT NULL,\n" +
